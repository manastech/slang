...
import * as bench from "tinybench";
...
async function run(name: string, solidityVersion: string, dir: string, file: string, expectedDefs?: number, expectedRefs?: number) {
  console.log(`Testing ${name}`);
  const suite = new bench.Bench({ name: name, iterations: 5 });
  suite.add("slang", async () => {
    await testFile(solidityVersion, [dir, file].join("/"), expectedDefs, expectedRefs);
  });
  suite.add("solc", async () => {
    await testFileSolC(solidityVersion, dir, file);
  });
  await suite.run();

  console.log(suite.table());
  return name;
}
